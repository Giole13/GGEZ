...
namespace WhatIsFunction
{
    public class MineGame
    {
        /**
         * 지회 찾기
         * 10 x 10 보드에 지뢰를 숨김 (n% 확률로 지뢰 매벌)
         * debug mode 에서 지뢰가 아닌 곳은 .(닷). 지뢰인 곳은 #(샵)으로 표현
         * play mode 에서 확인 되지 않는 곳은 전부 □(스퀘어) 로 표현
        ...
        public static void Main()
        {
            ...
            for (int y = 0; y < BOARD_SIZE_Y; y++)
            {
                for (int x = 0; x < BOARD_SIZE_X; x++)
                {
                    ...
                    if (gameBoard[y, x] < MINE_PRECETAGE)
                    {
                        mineCntMap[y, x] = -1;
                    }       //if: 지뢰가 셋업된 경우
                    else
            ...
            while (isGameOver == false)
            {
                ...
                while (isLocationValid == false)
                {
                    ...
                    {
                        Console.WriteLine("{0} {", "[System] 해당 좌표는 유효하지 않습니다.",
                            "다른 좌표를 입력하세요 \n");
                        continue;
                    }       //if: 좌표를 잘못 입력한 경우
                ...
                {
                    gameBoard[playerY, playerX] = MINE_PRECETAGE + 1;
                ...
                // { 선택한 좌표에서 지뢰를 검사한다.
                if (gameBoard[playerY, playerX] < MINE_PRECETAGE)
                {
                    isGameOver = true;
                    isPlayerWin = false;
                ...
                else
                {
                    ...
                    for (int searchY = playerY - 1; searchY <= playerY + 1; searchY++)
                    {
                        for (int searchX = playerX -1; searchX <= playerX + 1; searchX++)
                        {
                            ...
                            {
                                playBoard[searchY, searchX] = mineCntMap[searchY, searchY];
                            }       //else: 지뢰 아닌 경우
                        }
                ...
                for (int y = 0; y < BOARD_SIZE_Y; y++)
                {
                    ...
                    for (int x = 0; x < BOARD_SIZE_X; x++)
                    {
                        ...
                        {
                            unknowTileCnt++;
                        }       // if: 아직 오픈할 타일이 존재하고, 해당 타일이 지뢰가 아닌경우
                    }
                }       //loop: 플레이 보드를 순회하는 루프

                if (unknowTileCnt.Equals(0))
                {
                    isGameOver = true;
                    isPlayerWin = true;
                }
